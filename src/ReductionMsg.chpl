module ReductionMsg
{
    ...
    proc segmentedReductionMsg(reqMsg: string, st: borrowed SymTab): string throws {
      ...
      var rname = st.nextName();
      if v {try! writeln("%s %s %s %s %s".format(cmd,values_name,segments_name,operator, ignore_nan));try! stdout.flush();}
    ...
    proc segmentedLocalRdxMsg(reqMsg: string, st: borrowed SymTab): string throws {
      ...
      var rname = st.nextName();
      if v {try! writeln("%s %s %s %s %s".format(cmd,keys_name,values_name,segments_name,operator, ignore_nan));try! stdout.flush();}
      ...
      when (DType.Float64) {
        ...
        select operator {
          ...
          when "mean" {
            var res = perLocMean(values.a, segments.a, ignore_nan);
            st.addEntry(rname, new shared SymEntry(res));
          }
          when "min" {
            var res;
    ...
    */
    ...
    proc segSum(values:[] bool, segments:[?D] int, param ignoreNan = "True"): [D] int {
      var res: [D] int;
      if (D.size == 0) { return res; }

      var cumsum;
      cumsum = + scan values;
      //var cumsum = + scan ([elem in values] if isnan(elem) then 0.0 else elem);
      // Iterate over segments
      forall (i, r) in zip(D, res) {
    ...
    proc perLocSum(values:[] bool, segments:[?D] int, param ignoreNan = "True"): [] int {
      var numKeys:int = segments.size / numLocales;
      var keyDom = makeDistDom(numKeys);
      ...
      coforall loc in Locales {
        on loc {
          perLocVals[here.id] = segSum(values.localSlice[values.localSubdomain()],
                                       segments.localSlice[D.localSubdomain()], ignoreNan);
        }
      }
    ...
    proc segMean(values:[] ?t, segments:[?D] int, param ignoreNan = "True"): [D] real {
      var res: [D] real;
      if (D.size == 0) { return res; }
      var sums = segSum(values, segments, ignoreNan);
